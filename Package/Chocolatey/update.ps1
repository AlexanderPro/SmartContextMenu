$releases = 'https://github.com/AlexanderPro/SmartContextMenu/releases'

function global:au_GetLatest {
    $download_page = Invoke-WebRequest -Uri $releases
    $regex   = '/AlexanderPro/SmartContextMenu/releases/download/.*?/SmartContextMenu_v.*?.zip$'
    $url     = "https://github.com/" + ($download_page.links | ? href -match $regex | select -First 1 -expand href)
    
    $verregex= [regex]'/SmartContextMenu_v(.*?).zip$'
