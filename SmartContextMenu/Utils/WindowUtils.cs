...
using static SmartContextMenu.Native.Gdi32;
...
namespace SmartContextMenu.Utils
{
    static class WindowUtils
    {
        ...
        public static bool IsAltTabWindow(IntPtr hWnd)
        {
            ...
            var hwndTry = GetAncestor(hWnd, GetRootOwner);
        ...
        public static Icon GetIcon(IntPtr hWnd)
        {
            ...
            try
            {
                SendMessageTimeout(hWnd, WM_GETICON, ICON_SMALL2, 0, SMTO_ABORTIFHUNG, 100, out var result);
                icon = new IntPtr(result);

                if (icon == IntPtr.Zero)
                {
                    SendMessageTimeout(hWnd, WM_GETICON, ICON_SMALL, 0, SMTO_ABORTIFHUNG, 100, out result);
                    icon = new IntPtr(result);
                }

                if (icon == IntPtr.Zero)
                {
                    SendMessageTimeout(hWnd, WM_GETICON, ICON_BIG, 0, SMTO_ABORTIFHUNG, 100, out result);
                    icon = new IntPtr(result);
                }
